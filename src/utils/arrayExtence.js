import { isArray } from './jsTypeUtil'
...
const isEff1dArray = (obj) => {
  return isArray(obj) && obj.length > 0 && obj.some(item => item !== '' && item !== undefined && JSON.stringify(item) !== 'null')
}
...
const isNullStr2dArray = (obj) => {
  return isArray(obj) && obj.length > 0 && obj.every(item => isArray(item) && item.length > 0 && item.every(ele => ele === ''))
}
...
const getEff2dArrayByCols = (target2dArray, cols) => {
  if (!is2dArray(target2dArray)) throw new Error('params target2dArray is not a strict 2dArray')
  const $cols = cols || target2dArray.reduce((acc, item) => Math.max(acc, Number(getArrayTailNaEIndex(item) + 1)), 0)
...
const arrayAssign1d = (...target1dArrays) => {
  let args = deepCope(target1dArrays)
  if (args.some(arg => !isArray(arg))) throw new Error('type of oneArg is not array')
...
const arrayAssign2d = (...target2dArrays) => {
  ...
  if (args.some(arg => !is2dArray(arg))) throw new Error('one of target2dArrays is not a strict 2dArray')
  const reverseArgs = args.reverse()
  return reverseArgs.reduce((acc, item, index) => {
    if (index === 0) return acc.concat(item)
    else {
      let oneAssign2dArr = acc.map((eleArr, index) => arrayAssign1d(item[index] || [], eleArr))
